...
function initSyncAll() {
  ...
  async function loadLastSync() {
    ...
    if (textEl) {
      if (settings && settings.lastSync) {
        ...
        textEl.textContent = 'Last synced: ' + (isToday ? 'Today ' : d.toLocaleDateString() + ' ') + d.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      } else {
  ...
  if (syncAllBtn) {
    syncAllBtn.addEventListener('click', () => {
      ...
      if (btnText) btnText.textContent = 'Starting Sync...';

      chrome.runtime.sendMessage({ type: 'SYNC_ALL' }, (res) => {
        syncAllBtn.disabled = false;
        if (btnText) btnText.textContent = '\u21bb Sync All Sites';
        if (res && res.success) {
          showToast('\u2705 Synced 5 sites \u2014 ' + (res.newApps || 0) + ' new, ' + (res.updates || 0) + ' updates');
          loadJobs();
          loadLastSync();
        } else {
          showToast('\u274c Sync failed or no new data');
        }
      });
    });
  }

  // Listen for live progress messages from the background
  chrome.runtime.onMessage.addListener((message) => {
    if (message.type === 'SYNC_PROGRESS') {
      const btnText = document.getElementById('syncAllBtnText');
      if (btnText) btnText.textContent = message.text;
...
function createJobCard(job) {
  ...
  card.innerHTML = `
    ...
    <div class="job-right">
      ...
      <div style="display:flex;gap:4px;">
        <button class="match-btn" title="Check resume match">
        ...
        ${job.url && job.url !== 'chrome://newtab/' && !job.url.startsWith('chrome-extension') ? `
          <button class="link-btn" title="Open Job Page" data-url="${escHtml(job.url)}">
  ...
  if (matchBtn) {
    matchBtn.addEventListener('click', (e) => {
      ...
      const url = chrome.runtime.getURL('popup/match.html') +
        '?role=' + encodeURIComponent(job.role) +
        '&company=' + encodeURIComponent(job.company);
      chrome.windows.create({ url, type: 'popup', width: 580, height: 720 });
    });
  }
...
function setupEventListeners() {
  ...
  document.getElementById('settingsBtn').addEventListener('click', () => {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      if (currentTab) {
        const isNewTab = currentTab.url === 'chrome://newtab/' || currentTab.url === 'edge://newtab/' || currentTab.pendingUrl === 'chrome://newtab/';
        const isExtensionTab = currentTab.url.startsWith(chrome.runtime.getURL(''));
        
        if (isNewTab || isExtensionTab) {
          chrome.tabs.update(currentTab.id, { url: chrome.runtime.getURL('options/options.html') });
          return;
        }
      }
  ...
  // Open in full tab buttons
  const expandTabBtn = document.getElementById('expandTabBtn');
  const settingsOpenTabBtn = document.getElementById('settingsOpenTabBtn');
  const openFullTab = () => {
  ...
  chrome.tabs.getCurrent((tab) => {
    if (tab) {
      if (expandTabBtn) expandTabBtn.style.display = 'none';
      if (settingsOpenTabBtn) settingsOpenTabBtn.style.display = 'none';
    } else {
      if (expandTabBtn) expandTabBtn.addEventListener('click', openFullTab);
      if (settingsOpenTabBtn) settingsOpenTabBtn.addEventListener('click', openFullTab);
    }
  });
}
...
function showToast(msg) {
  ...
  if (!t) {
    ...
    t.id = '_toast';
    t.style.cssText = 'position:fixed;bottom:60px;left:50%;transform:translateX(-50%);background:var(--dark);color:var(--bg);padding:7px 16px;border-radius:20px;font-size:12px;font-weight:600;z-index:9999;pointer-events:none;transition:opacity 0.3s;white-space:nowrap;';
